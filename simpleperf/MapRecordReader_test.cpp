...
TEST_F(MapRecordReaderTest, MapRecordThread) {
  ...
  ASSERT_TRUE(CreateMapRecordReader());
  MapRecordThread thread(*reader_, nullptr);
...
// @CddTest = 6.1/C-0-2
TEST_F(MapRecordReaderTest, MapRecordThread_BinaryFilter) {
#ifdef __ANDROID__
  std::string tmpdir = "/data/local/tmp";
#else
  std::string tmpdir = "/tmp";
  ...
  // Use a filter that matches libc.so.
  MapRecordThread thread(*reader_, R"(libc\.so)");
  ASSERT_TRUE(thread.Join());
  ...
  auto records = ReadRecordsFromBuffer(attr_, buffer.data(), buffer.size());
  size_t mmap_count = 0;
  size_t mmap2_count = 0;
  for (const auto& r : records) {
    if (r->type() == PERF_RECORD_MMAP) {
      mmap_count++;
    } else if (r->type() == PERF_RECORD_MMAP2) {
      std::string_view s(static_cast<Mmap2Record*>(r.get())->filename);
      ASSERT_TRUE(s.find("libc.so") != s.npos);
      mmap2_count++;
    }
  }
  // Kernel maps (MMAP) should be preserved regardless of the filter.
  ASSERT_GT(mmap_count, 0);
  // Only user maps (MMAP2) matching the filter should be kept.
  ASSERT_GT(mmap2_count, 0);
}
